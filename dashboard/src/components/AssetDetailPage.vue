...
const assetApi = useApi<Asset>(() => getAsset(assetId.value));
const movementsApi = useApi<{ movements: Movement[]; total: number }>(() =>
  getMovements(assetId.value, { limit: 50 })
);
const snapshotsApi = useApi<{ snapshots: AssetSnapshot[] }>(() =>
  getAssetSnapshots(assetId.value)
);
...
async function loadMoreMovements() {
  try {
    const data = await getMovements(assetId.value, { limit: 50, offset: movementOffset.value });
    movements.value = [...movements.value, ...data.movements];
    movementOffset.value = movements.value.length;
  } catch { /* noop */ }
}
...
const MOVEMENT_BADGE_CLASSES: Record<string, string> = {
  ...
  adjustment: 'bg-indigo-500/10 text-indigo-500',
};

const MOVEMENT_TYPES = ['deposit', 'withdrawal', 'buy', 'sell', 'dividend', 'fee', 'adjustment'];
...
async function saveHolding() {
  ...
  try {
    if (editingHolding.value) {
      ...
      await createHolding(assetId.value, {
    ...
    <template v-else-if="asset">
      ...
      <div class="space-y-4">
        ...
        <div v-else class="space-y-0 border border-border rounded-md divide-y divide-border">
          <div v-for="m in movements" :key="m.id" class="px-4 py-3 group">
            <div class="flex items-start justify-between">
              <div>
                <div class="flex items-center gap-2">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <Select v-model="movementForm.type">
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem v-for="t in MOVEMENT_TYPES" :key="t" :value="t">{{ t }}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
