...
The Mac mobile API stays intentionally small: bootstrap for identity/freshness, overview for available data months plus Mac-calculated cashflow, pace, categories, merchants, budgets, visit changes, and net worth, plus transaction reads and narrowly scoped review writes. Home renders the selected overview as an interactive pace/category carousel. Explore loads up to six available month-scoped overview responses ending at its independently selected month, with deterministic category → merchant → transaction drill-down. Activity uses a native `TextInput` and `SectionList`, and maps its filter sheet directly onto the transaction contract while keeping owner/inclusion refinement local. Review keeps its haptics and one-at-a-time progress while sharing a searchable, compact two-column category sheet with transaction detail.

The Blue Ledger interaction layer stays local and small: semantic color roles live in `src/theme.ts`; Expo Router native tabs own the iOS tab material and minimization; `GlassSegmentedControl` owns the shared Explore/net-worth range lens; and `CategoryPickerSheet` owns the compact searchable selector. Shared drill-downs are registered on the root stack, so a native back swipe restores the originating tab and its exact scroll state instead of redirecting to the screen's former owning tab. `expo-glass-effect` supplies Liquid Glass for range controls without a UI framework.
