...
export type CanonicalHomeHarnessOptions = Parameters<typeof createCanonicalHarness>[0] & {
  accountBalance?: number | null;
};

export async function createCanonicalHomeHarness(
  options: CanonicalHomeHarnessOptions = {},
): Promise<CanonicalHomeHarness> {
  const { accountBalance = 8200, ...canonicalOptions } = options;
  ...
  try {
    const harness = await createCanonicalHarness({ ...canonicalOptions, sqlite: testDb.sqlite });
