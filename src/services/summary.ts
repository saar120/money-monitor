...
export function getSpendingSummary(
  ...
  if (empty) {
    if (groupBy === 'cashflow-detail') {
      return {
        ...
        summary: { income: [], expenses: [], totalIncome: 0, totalExpenses: 0, surplus: 0 },
      };
    }
  ...
  if (groupBy === 'cashflow') {
    const rows = db
      .select({
        ...
        expense:
          sql<number>`SUM(CASE WHEN ${transactions.chargedAmount} < 0 THEN ABS(${transactions.chargedAmount}) ELSE 0 END)`.as(
            'expense',
          ),
      })
      .from(transactions)
  ...
  if (groupBy === 'cashflow-detail') {
    const income = db
      ...
      .from(transactions)
      .where(and(...conditions, sql`${transactions.chargedAmount} > 0`))
...
export function comparePeriods(input: {
  ...
  const comparison = [...allCategories]
    .map((category) => {
      ...
      return {
        category,
        period1_total: round2(t1),
        period1_count: c1,
        period2_total: round2(t2),
        period2_count: c2,
  ...
  return {
    ...
    summary: {
      ...
      change_percent:
        p1Total !== 0 ? Math.round((overallChange / Math.abs(p1Total)) * 100 * 10) / 10 : null,
    },
  };
}
...
export function getTopMerchants(input: {
  ...
  const groups = new Map<
    string,
    Array<{ amount: number; date: string; category: string | null }>
  >();
  for (const row of rows) {
    const key = normalizeDescription(row.description);
    if (!groups.has(key)) groups.set(key, []);
  ...
  let merchants = [...groups.entries()].map(([merchant, entries]) => {
    ...
    const topCategory =
      [...catCounts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? 'uncategorized';
