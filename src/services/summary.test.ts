...
const { getSpendingSummary, comparePeriods, getTopMerchants } = await import('./summary.js');
...
describe('summary service', () => {
  ...
  describe('getSpendingSummary', () => {
    ...
    it('groups by category', () => {
      ...
      const food = rows.find((s) => s.category === 'food');
    ...
    it('groups by cashflow showing income and expense', () => {
      ...
      const jan = (result.summary as any[])[0] as {
        month: string;
        income: number;
        expense: number;
      };
    ...
    it('groups by cashflow-detail with income and expenses by category', () => {
      ...
      insertTransaction(testDb.db, account.id, { category: 'freelance', chargedAmount: 2000 });
      insertTransaction(testDb.db, account.id, { category: 'food', chargedAmount: -1000 });
      insertTransaction(testDb.db, account.id, { category: 'food', chargedAmount: -500 });
      insertTransaction(testDb.db, account.id, { category: 'transport', chargedAmount: -300 });

      const result = getSpendingSummary({}, 'cashflow-detail');
      expect(result.groupBy).toBe('cashflow-detail');
      ...
      expect(summary.income).toHaveLength(2);
      expect(summary.expenses).toHaveLength(2);

      const salary = summary.income.find((i) => i.category === 'salary');
      expect(salary).toBeDefined();
      expect(salary!.amount).toBe(5000);

      const food = summary.expenses.find((e) => e.category === 'food');
      expect(food).toBeDefined();
      expect(food!.amount).toBe(1500);

      expect(summary.totalIncome).toBe(7000);
      expect(summary.totalExpenses).toBe(1800);
      expect(summary.surplus).toBe(5200);
    });

    it('cashflow-detail returns empty object shape when no transactions', () => {
      const result = getSpendingSummary({}, 'cashflow-detail');
      expect(result.groupBy).toBe('cashflow-detail');
      ...
      expect(summary.income).toHaveLength(0);
    ...
    it('cashflow-detail coalesces null categories to uncategorized', () => {
      const account = insertAccount(testDb.db);
      insertTransaction(testDb.db, account.id, { category: null, chargedAmount: 1000 });
      insertTransaction(testDb.db, account.id, { category: null, chargedAmount: -500 });
      ...
      expect(summary.income[0].category).toBe('uncategorized');
      expect(summary.expenses[0].category).toBe('uncategorized');
    });

    it('cashflow-detail returns empty object when accountType has no matching accounts', () => {
      const account = insertAccount(testDb.db, { accountType: 'bank' });
      insertTransaction(testDb.db, account.id, { chargedAmount: 5000 });

      const result = getSpendingSummary({ accountType: 'credit_card' }, 'cashflow-detail');
      const summary = result.summary as {
        income: any[];
        expenses: any[];
        totalIncome: number;
    ...
    it('filters by date range', () => {
      ...
      const total = (result.summary as any[]).reduce(
