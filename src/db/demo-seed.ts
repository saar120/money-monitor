...
export function seedDemoData(
  ...
  sqlite.transaction(() => {
    const memberA =
      db.select({ id: schema.members.id }).from(schema.members).orderBy(schema.members.id).get() ??
      db
        .insert(schema.members)
        .values({ name: 'Member 1' })
        .returning({ id: schema.members.id })
        .get();
    const memberB = db
      .insert(schema.members)
      .values({ name: 'Member 2' })
      .returning({ id: schema.members.id })
      .get();
    const demoMemberIds = [memberA.id, memberA.id, memberA.id, memberB.id];
    ...
    const accountRows = [
      {
        companyId: 'hapoalim',
        displayName: 'Bank Hapoalim - Checking',
        accountNumber: '123456',
        accountType: 'bank' as const,
        balance: 24350.8,
        credentialsRef: dummyRef,
      },
      {
        companyId: 'leumi',
        displayName: 'Bank Leumi - Savings',
    ...
    for (const a of accountRows) {
      const result = db
        ...
        .values({
          ...a,
          memberId: demoMemberIds[accountIds.length],
          isActive: true,
    ...
    const txTemplates: Array<{
      ...
      // Groceries
      {
        description: 'שופרסל דיל - סניף העיר',
        category: 'groceries',
        minAmount: -80,
        maxAmount: -450,
        accountIdx: 2,
      ...
      {
        description: 'תן ביס - ארוחת צהריים',
        category: 'restaurants',
        minAmount: -35,
        maxAmount: -75,
        accountIdx: 3,
      ...
      {
        description: 'פלאפון - חשבון חודשי',
        category: 'utilities',
        minAmount: -60,
        maxAmount: -120,
        accountIdx: 2,
      ...
      // Fitness
      {
        description: 'Holmes Place - מנוי חודשי',
        category: 'fitness',
        minAmount: -280,
        maxAmount: -350,
        accountIdx: 2,
      ...
      {
        description: 'פייבוקס - כרטיסי הופעה',
        category: 'entertainment',
        minAmount: -150,
        maxAmount: -400,
        accountIdx: 2,
      ...
      for (const fixed of monthlyFixed) {
        ...
        db.insert(schema.transactions)
          .values({
            ...
            expenseOwnerMemberId: demoMemberIds[fixed.accountIdx],
            ownerSource: 'account',
            ownerConfidence: 1,
            confidence: 0.95,
            hash: randomUUID(),
    ...
    for (let monthBack = 0; monthBack < 6; monthBack++) {
      ...
      for (let i = 0; i < txPerMonth; i++) {
        const template =
          txTemplates[Math.floor(seededRandom(monthBack * 1000 + i) * txTemplates.length)];
        ...
        const amount =
          Math.round((template.minAmount + seededRandom(monthBack * 3000 + i) * range) * 100) / 100;
        const dateStr = formatDate(txDate);
        ...
        db.insert(schema.transactions)
          .values({
            ...
            expenseOwnerType: 'member',
            expenseOwnerMemberId: demoMemberIds[template.accountIdx],
            ownerSource: 'account',
            ownerConfidence: 1,
            confidence: 0.9,
            hash: randomUUID(),
    ...
    const holdingsData = [
      {
        ...
        costBasis: 58500,
        lastPrice: 4250,
        lastPriceDate: formatDate(daysAgo(1)),
      },
      {
        assetId: brokerage.id,
        name: 'אג"ח ממשלתי 0526',
        type: 'bond',
        currency: 'ILS',
        quantity: 50000,
        costBasis: 48500,
    ...
    for (let monthBack = 0; monthBack < 6; monthBack++) {
      ...
      db.insert(schema.assetSnapshots)
        .values({
          assetId: brokerage.id,
          date: dateStr,
    ...
    for (let weekBack = 0; weekBack < 26; weekBack++) {
      ...
      db.insert(schema.accountBalanceHistory)
        .values({
