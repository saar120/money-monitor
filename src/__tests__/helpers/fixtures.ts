...
export function insertMember(db: Db, overrides: Partial<typeof schema.members.$inferInsert> = {}) {
  return db
    .insert(schema.members)
    .values({
      name: overrides.name ?? `Member ${randomUUID().slice(0, 8)}`,
      ...overrides,
    })
    .returning()
    .get();
...
export function insertLiability(
  db: Db,
  overrides: Partial<typeof schema.liabilities.$inferInsert> = {},
) {
  return db
...
export function insertAssetSnapshot(
  db: Db,
  assetId: number,
  overrides: Partial<typeof schema.assetSnapshots.$inferInsert> = {},
) {
  return db
...
export function insertBalanceHistory(
  db: Db,
  accountId: number,
  overrides: Partial<typeof schema.accountBalanceHistory.$inferInsert> = {},
) {
  return db
