...
export function buildUpdateAlertSettingsTool() {
  return createAgentTool({
    ...
    parameters: Type.Object({
      ...
      daily_digest_enabled: Type.Optional(Type.Boolean({ description: 'Enable post-scrape daily digest' })),
      daily_digest_large_charge_threshold: Type.Optional(Type.Number({ description: 'Minimum amount to flag as a large charge (positive number, e.g. 500)' })),
      daily_digest_report_errors: Type.Optional(Type.Boolean({ description: 'Report scrape failures in digest' })),
      unusual_spending_enabled: Type.Optional(Type.Boolean({ description: 'Enable unusual spending alerts' })),
      unusual_spending_percent_threshold: Type.Optional(Type.Number({ description: 'Percentage increase to trigger alert (e.g. 30 = 30% spike)' })),
      new_recurring_enabled: Type.Optional(Type.Boolean({ description: 'Enable new recurring charge detection' })),
      review_reminder_enabled: Type.Optional(Type.Boolean({ description: 'Enable low-confidence categorization reminders' })),
      monthly_summary_enabled: Type.Optional(Type.Boolean({ description: 'Enable monthly summary on 1st of month' })),
      net_worth_change_enabled: Type.Optional(Type.Boolean({ description: 'Enable net worth change alerts' })),
      net_worth_change_threshold: Type.Optional(Type.Number({ description: 'Minimum ILS change to trigger net worth alert' })),
    ...
    execute: async (args) => {
      ...
      if (args.new_recurring_enabled !== undefined) patch.newRecurring = { enabled: args.new_recurring_enabled };
      if (args.review_reminder_enabled !== undefined) patch.reviewReminder = { enabled: args.review_reminder_enabled };
