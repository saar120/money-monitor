...
| P2-IOS-01 | iOS models/formatting | In progress | Phase 2A implements the Home formatting subset against the accepted bootstrap DTO; transaction DTO work still depends on P2-API-01. Full spoken-wording acceptance remains deferred. |
| P2-IOS-02 | iOS Home | In progress | Build live, memory-only Home from the validated bootstrap. Phase 2A suppresses failed sections and does not claim cached/offline behavior; the full repository-backed task remains dependent on Phase 1. |
...
| P2-DES-01 | Design + UX writing | In progress | Phase 2A covers live/loading/refreshing/partial/unavailable/revoked/incompatible Home states; cached/stale and remaining-screen matrices stay planned for the full phase. |
...
| P2-QA-01 | QA | In progress | Phase 2A reuses bootstrap fixtures for Home redaction/decoding and adds memory-only/privacy-cover proofs; transaction contract and snapshot coverage remain planned. |
