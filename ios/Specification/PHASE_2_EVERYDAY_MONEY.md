...
- authenticated shared `GET /api/v1/transactions` with the same allow-listed contract on Mac and iPhone;
- authenticated `GET /api/v1/transactions/:id`, where `:id` is an opaque transaction ID and the response contains no editable affordance;
...
- in-memory query, applied-filter, result, cursor, scroll, and detail state that is cleared on revocation/disconnect; the latest 200 accepted rows are saved atomically for offline browsing and search;
...
- persistence beyond the encrypted, bounded 200-row Saved View snapshot;
...
| P2-DAT-01 | iOS data                       | Implemented                                       | The encrypted snapshot repository atomically retains the latest accepted complete response and bounds Saved Activity/Search to 200 transactions; search recents remain out of scope.                                                            |
| P2-DES-01 | Design + UX writing            | Accepted                                          | The flat native hierarchy covers live Activity/Search/filter/detail plus explicitly labeled Saved and stale states; older history clearly requires Live.                                                                                       |
...
| P2-QA-01  | QA                             | Accepted                                          | Fixture, refresh, privacy-boundary, normalization, generated-client, encrypted Saved snapshot, and simulator integration coverage pass without persisting query/filter/cursor state or logging financial data.                                  |
| P2-QA-02  | QA                             | Accepted                                          | Live/empty/partial/Saved/stale, revocation, malformed data, duplicate/cursor abuse, cancellation, append error/retry, race protection, and privacy-cover paths are deterministic.                                                               |
