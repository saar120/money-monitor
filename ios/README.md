...
| Private Mac bridge                          | Loopback server, path-scoped Tailscale coordinator, readiness preflight, and crash-safe process cleanup implemented; live Tailnet route confirmed               |
...
| Stable Mac pairing endpoint                 | Claimant-bound public routes, explicit approval, digest-only registry, atomic re-pair rotation, and native Keychain client implemented; physical pairing passed |
